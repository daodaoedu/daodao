...
| 私密活動旗標的行為 | 本輪強制 `is_private`（影響 cohort 實踐的 `privacy_status`）；「打卡預設私密」「發起人留言預設私密」存值＋顯示，不強制（需要逐則打卡隱私欄位，另案） |
...
- `activity-discovery`：「探索活動頁公開列表端點」每筆回傳欄位增加一句話簡介、互動方式、收費資訊；探索頁篩選增加線上／實體
