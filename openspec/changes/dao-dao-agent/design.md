...
每個 Turn 的 LLM 呼叫、tool_call（工具名 + 參數摘要 + 結果摘要）、approval 決策（模式、allow/deny、操作者）、Skill 觸發與升格 MUST 寫入 audit log（新增 `agent_audit_log` 表），可依 thread / user / 時間區間查詢。audit 記錄本身遵循 PII 規範（不落 email / phone 明文）。
...
- 是否採用第三方開源方案（AnythingLLM、Open WebUI、LibreChat 等）作為對話前端或參考架構？**進行中：由 Tim 負責調研，結論回寫本文件。**
