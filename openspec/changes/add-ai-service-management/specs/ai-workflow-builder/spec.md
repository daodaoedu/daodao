...
系統 SHALL 允許 admin 在 Workflow 中新增 Node，每個 Node 必須選擇類型並填入對應 config。Phase 1 開放的 Node 類型：`llm-call`、`data-fetch`、`data-transform`、`tool-call`、`skill-call`、`output`（`condition` 預留，Phase 1 不開放）。
...
- **WHEN** admin 選擇類型 `skill-call`，從下拉清單選擇已建立的 Skill，選擇 provider 並填入 input template
- **THEN** 系統建立 Node，卡片顯示 Skill 名稱與 provider

#### Scenario: skill-call Node 執行
- **WHEN** Workflow 執行到 `skill-call` Node
- **THEN** daodao-server 呼叫 `POST /internal/execute/skill-call`（daodao-ai-backend），ai-backend 載入 Skill 的 SKILL.md 與 scripts/ 工具，以 ReAct agent loop 執行後回傳輸出
