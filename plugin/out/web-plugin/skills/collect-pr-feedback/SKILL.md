...
- Commit 遵守目標 repo 的 **pre-commit-check**（`skills/pre-commit-check/SKILL.md`，未隨本包出貨）、**format-commit**（`skills/format-commit/SKILL.md`，未隨本包出貨） 與確認規則。Commit 授權不等於 push、merge 或回覆 reviewer 的授權。
