...
## ⚠ 執行環境落差（OpenAI Plugin（ChatGPT 網頁／行動版＋Codex））

這個 plugin 會同時被有 repo 的 Codex 與沒有 repo 的 ChatGPT 網頁／行動版載入。**先確認你手上有沒有下列能力**；沒有的部分照著做但不要宣稱已完成，改成輸出給人執行的指令，並在交付時明列哪幾項未查證：

- **browser**（瀏覽器自動化驗證與截圖）：沒有瀏覽器工具時，畫面驗證標記為未完成，不得用讀程式碼代替。
- **drive**（上傳 Google 文件／Drive）：沒有 Drive connector 時，報告留在本機或直接貼進對話。
- **hooks**（harness 自動觸發的攔截式 gate）：plugin 內沒有附 hooks。發 PR 前的閘門必須由 agent 主動逐項執行並貼出輸出，不得宣稱機器已攔截。
