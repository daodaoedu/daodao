...
CLI 包含 tracked staged／unstaged；新增測試尚未 staged 時需另讀檔，完成已授權提交後再用 `--head HEAD` 核對。新增 skip／focus／todo 阻擋；刪除或替換 assertion／test 要查明原因並交人審核具體 diff。依 **工具說明**（repo 檔案 `scripts/check-test-integrity.md`，未隨本包出貨） 記錄綁定 base 與 test diff digest 的審核依據，不自行製造人工 receipt。此 heuristic 不能抓所有語意弱化，仍需跑測試並檢查內容。
