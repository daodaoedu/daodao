...
if ! jq --version >/dev/null 2>&1; then
  if  printf '%s' "$HOOK_RAW" | grep -qE 'gh[[:space:]]+pr[[:space:]]+create'; then
    echo "❌ pre-pr-gate 需要 jq 解析工具輸入，但找不到 jq；閘門無法判斷就不放行（brew install jq / apt install jq）" >&2
    exit 2
  fi
  exit 0
fi
cmd=$(echo "$HOOK_TOOL_INPUT" | jq -r '.command // empty' 2>/dev/null || true)
[ -z "$cmd" ] && exit 0
...
❌ 這個任務有可互動的 POC（$task_dir/poc/），但還沒做「POC 並排量測比對」就要發 $repo_dir 的 PR。
   ...
   做法見 $PLUGIN_ROOT/skills/dev-task/references/poc-compare.md
...
❌ 核心旅程矩陣沒過：$problems
   ...
   格式與輸入目錄見 $PLUGIN_ROOT/skills/dev-task/references/journey-matrix.md
...
❌ 版面探針沒過：$probe_problem
   在 ${task_dir}/${repo_dir}/apps/product（f2e）或 ${task_dir}/${repo_dir}（admin-ui）底下跑：
     node $PLUGIN_ROOT/skills/dev-task/references/layout-probe.mjs --base http://localhost:<port> \\
