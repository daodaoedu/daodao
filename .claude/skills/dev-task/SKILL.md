...
2. **逐 phase 驗收** — 從 task.md 的 phases + 已確認的驗收契約展開檢查清單，用瀏覽器實際走過每一條：
   ...
   - **版面 rect 對照**（零視覺差異的 refactor 必做）：抽共用元件／換 className／搬 DOM 時，layout-probe 只證明「沒有溢出」，不證明「版面沒變」。用 `references/rect-probe.mjs` 改前改後各量一次、`rect-diff.py` 比對 rect 與 computedStyle（見 [browser-verify.md §4b](references/browser-verify.md)），差異表貼進 task.md。#240 抽 `PageShell` 就是這樣證明 15 頁 45 組零差異的
