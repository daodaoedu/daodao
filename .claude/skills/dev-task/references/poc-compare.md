...
cp .claude/skills/dev-task/references/poc-compare/poc-compare.mjs "$TASK/daodao-f2e/apps/product/e2e-scratch/"
# 2. 依 config.example.mjs 寫 $TASK/notes/poc-compare/config.mjs（checkpoints：flow + probes；probe key 前綴 = 類別）
cp .claude/skills/dev-task/references/poc-compare/config.example.mjs "$TASK/notes/poc-compare/config.mjs"
...
python3 .claude/skills/dev-task/references/poc-compare/poc-report.py --task "$TASK"
```
