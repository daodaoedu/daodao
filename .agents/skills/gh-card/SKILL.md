...
先讀 [AI 檢核與人工審核共用流程](../../../plugin/docs/ai-human-review-workflow.md)，依當前客戶端可用工具執行；先完成適用檢核與修訂，再交人審核決策。
...
- 中央 feature／工作卡：讀 [模板規則](../../../plugin/templates/README.md)與 [central-issue.md](../../../plugin/templates/central-issue.md)。
- 明確要求拆子 Issue：另外讀 [subtask-issue.md](../../../plugin/templates/subtask-issue.md)，引用中央 AC；保留獨立行 `Parent: daodaoedu/daodao#N`。中央尚未建立時先存子卡草稿，取得實際號碼後再發布。
...
預告交付欄位依 [issue-status-comment.md](../../../plugin/templates/issue-status-comment.md)：PR、驗收報告、截圖、API／POC 結果、未完成範圍。自動回寫器未部署時明講需由開發流程 finish 執行，開卡不代表已有全自動開發能力。
