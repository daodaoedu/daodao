...
Skill 是對**重複使用的業務邏輯**的封裝——一個 Python 工作流模組，包含資料查詢、LLM 生成、業務執行的完整流程。

- 新需求：直接對話描述，Agent 臨時組合工具完成
- 穩定需求：封裝成 Skill，之後對話一句話觸發
- 高頻需求：Skill 升級為排程，自動週期ul4執行
...
|------|---------|-------------------|
| **Model** | CPU | LLMClient 後端（gemini / groq / anthropic…）— 提供原始推理能力 |
...
| `daodao-pg::describe_schema`       | MCP pg            | 查詢資料表結構               |
| `daodao-pg::get_user_full_context` | MCP pg            | 取得用戶完整學習脈絡         |
...
| 工具                                    | 來源              | 用途                         |
| --------------------------------------- | ----------------- | ---------------------------- |
| `LLMClient.generate(prompt, backend)` | daodao-ai-backend | 自由格式生成，backend 可指定 |
...
| `gh api`        | GitHub CLI       | 查詢 / 建立 issue、PR          |
| `stealth_fetch` | MCP              | 抓取外部網頁                   |

---

## 5. Skill 清單

Skill 是已封裝的業務邏輯模組，放在 `daodao-ai-backend/src/services/agent/skills/`。
...
> Skill 會隨使用增長。業務邏輯確認後封裝，不預設範圍。

### Skill 規格文件

詳細的 SQL、LLM prompt 規格、欄位對應：
...
```
你：切換到 anthropic，這份月報我想要文筆細膩一點

Agent → 切換 LLMClient backend 為 anthropic
     → 執行 monthly_insights skill
     → 所有 LLM 生成改用 claude-3-5-sonnet-20241022
```
