...
- 每個新頁面建立對應的 `.test.tsx` 檔案，測試渲染 + loading state + error state
- 每個 custom hook 建立對應的測試，mock API response 驗證資料轉換
- 共用元件（ExportButton、StatCard、SidebarGroup）建立獨立測試
- API functions 不額外測試（薄 wrapper），由 hook 測試覆蓋
- 使用 Vitest + @testing-library/react
- 每個 Phase 完成時做一次手動整合測試，確認所有路由可訪問

## Data Migration Plan

- **信任等級**：上線時所有既有用戶根據歷史活動資料（註冊天數、發文數、登入天數）一次性計算初始等級，不全部從 Lv0 開始
- **徽章**：上線時根據歷史資料回溯頒發符合條件的徽章（首次打卡、累計 N 篇筆記等）
- **排行榜**：上線時從歷史活動計算初始分數，「本週/本月」從上線日起算
- **用戶標籤**：自動標籤規則上線後立即執行一次全量掃描

## Additional Dependencies

- `@tiptap/react` + `@tiptap/starter-kit`：觸發式郵件模板和 FAQ 內容的 rich text 編輯器
- `vitest` + `@testing-library/react`：測試（如尚未安裝）
