...
對每個 repo 執行：
  gh issue list --repo daodaoedu/<repo> --label auto --state open \
    --json number,title,labels --limit 3
...
【步驟 2.3：實作（你自己寫 code）】
  ...
  state=needs-spec:
    ...
    2. git push → gh pr create --title "[spec] <title>" --label "auto,spec-pending"
    3. gh issue edit <num> --repo daodaoedu/<repo> --add-label spec-pending
    4. 結束此 issue（等 spec merge 後下輪再處理 code）
  state=needs-code:
...
【步驟 2.4：驗證】
  ...
  若成功：
    ...
    gh pr create \
      ...
      --head $BRANCH --base <default-branch> --label auto
