...
3. **模板不開新表**：`practice_templates` 加 `organization_id`（NULL＝官方模板），避免兩套模板結構漂移。
...
```sql
-- organization：開營組織（燈塔的主體；個人講師＝單成員組織）
CREATE TABLE organization (
  id            SERIAL PRIMARY KEY,
  name          VARCHAR(100) NOT NULL,         -- 品牌名或個人名
  bio           TEXT,
  external_link TEXT,                          -- 模板頁「了解更多」導流用
...
-- organization_members：組織成員（燈塔登入門檻＝EXISTS 此表）
...
-- 既有表修改（均為加欄位，不動既有資料）
ALTER TABLE practice_templates ADD COLUMN organization_id INT REFERENCES organization(id);
...
| 7 | 開營者採組織模型（organization ＋ organization_members） | 支援多人共管品牌；個人＝單成員組織；登入門檻＝成員資格（使用者定案） |
...
| 11 | 官方共同挑戰＝島島官方自己註冊為一個組織 | 同容器零特例，`programs.organization_id NOT NULL` 不需放寬 |

## Server 層對應（摘要）

```
/api/v1/lighthouse/*   組織端：programs/cohorts/templates/enrollments/stats
                       middleware: requireOrganizationMember（EXISTS organization_members）
                                 ＋物件級 ownership（cohort→program→organization→members）
