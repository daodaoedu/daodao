...
> **狀態校準（2026-07-06）：✅ 已上線。** 本文件的需求與設計內容仍有效，但許願池與公開 Roadmap 已實作於程式碼——規劃時請以程式碼為準，勿重做。實作位置：`daodao-server/src/routes/wish.routes.ts`、`daodao-server/src/routes/roadmap.routes.ts`、`daodao-f2e/packages/api/src/services/roadmap.ts`、product app 的 `/roadmap` 頁面。狀態追蹤見 daodao 主 repo 的 `scripts/product_status_manifest.yml` 與 `plugin/skills/product-status-check`。
