...
## 五、決策紀錄（2026-07-20，產品負責人拍板）

| 決策 | 結果 |
|---|---|
| D1 參與者保護包 | **自助退出／轉個人實踐：本階段做**（解除第三節第 2 項的內部矛盾）；**「僅自己」勾選：緩議**，定為第一批梯次結束後參與者訪談的必問題 |
| D2 同儕維度 | **做完整梯次動態牆**（參與者互見打卡並可回應）——推翻 FRD v0.3 的純單向匯報結構 |
| D3 管理端載體 | **定案（2026-07-21）：product app 內獨立 route group，產品名「燈塔」（URL `/lighthouse`）**——夥伴體驗完全分離（自有 layout＋登入門檻，gating 依 `organizers` 開通紀錄），零新增部署單位，五步流程直接複用；子網域 rewrite 為後續可選門面。已否決：admin.daodao.so 給夥伴登入（內部後台不對外是紅線）、獨立部署 app（成本無對價）；名稱 partner（撞 Buddy）與 coach 均否決，命名取島嶼意象（燈塔不拉船、只發光＝陪跑不監控） |
| D4 加入方式 | **email 名單＋邀請連結/QR 皆做**；名單來源合法性責任明文歸夥伴 |

**D1×D2 交互作用（強制配套）**：完整動態牆＋無「僅自己」＝歸屬梯次的打卡對教練與全體同梯可見且無單則隱藏。因此 (a) 加入同意文案必須明講「對教練與同梯參與者可見」；(b)「僅自己」訪談題不可省——牆使可見範圍從一位教練擴大到整個梯次，自我審查風險升高。

**D3 載體分析（留給工程拍板）**：

| 比較項 | f2e monorepo 新 app（apps/partner） | admin-ui 加 partner 角色 |
|---|---|---|
| Practice 五步建立流程（FRD 7.4.1 要求複用） | 直接 import product app 元件 | 技術棧不同（Vite vs Next.js），整段重寫 |
| API 型別同步 | openapi 自動生成（sync-openapi 現成） | types.ts 千行手動維護 |
| 安全邊界 | 內部後台不對外 | **內部管理後台對外部使用者開放**——nav 過濾即防線，且 admin.daodao.so 攻擊面變大 |
| 品牌/體驗 | 共用 packages/ui，島島產品觀感 | 內部工具 shadcn 觀感 |
| 儀表板圖表元件 | 需自建（f2e 統計元件少） | recharts/表格/匯出現成（唯一優勢） |
| 登入門檻（FRD 4.2） | 新 app 乾淨實作 | 需動 admin/superadmin 硬編碼白名單 |

review 建議傾向 f2e monorepo 新 app：admin-ui 唯一優勢（圖表元件）是最易補的，輸掉的兩項（五步重寫、內部後台對外）一是最大工程件、一是安全紅線。
